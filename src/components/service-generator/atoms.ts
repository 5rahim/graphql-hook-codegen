...
// `))
const graphql_action_file_content_atom = withImmer(atom(`
const TASK_FRAGMENT = graphql(\`
  fragment Task on service_tasks {
      id
      name
      done
    }
...
const GET_TASKS = graphql(\`
  query GetTasks {
    service_tasks(order_by: { createdAt: desc }) {
      ...Task
    }
  }
\`)
...
const CREATE_TASK = graphql(\`
  mutation CreateTask($task: service_tasks_insert_input!) {
    insert_service_tasks(objects: [$task]) {
      affected_rows
      returning {
        id
        name
...
const UPDATE_TASK = graphql(\`
  mutation UpdateTask($id: uuid!, $task: service_tasks_set_input!) {
    update_service_tasks_one(pk_columns: { id: $id }, _set: $task) {
      id
      name
      done
    }
...
const DELETE_TASK = graphql(\`
  mutation DeleteTask($id: uuid!) {
    delete_service_tasks_one(id: $id) {
      id
    }
  }
\`)
...
// @ts-ignore
const dynamic_parsed_content_atom = withImmer(atom<GraphQLParsedContent>({
   fields: [],
   keywords: [],
   actions: [],
   fragmentNames: [],
...
export const serviceGenerator_useParsedContent = () => {
   ...
   const updateAction = useCallback((id: string, target: keyof GraphQLAction, value: any) => {
      setDynamicParsedContent(prev => {
      const clonedContent = _.cloneDeep(prev)
         return _.update(clonedContent, `actions[${_.findIndex(clonedContent.actions, n => n.id === id)}].${target}`, v => value)
      })
   ...
   const updateActionVariable = useCallback((id: string, variableName: string, target: keyof GraphQLAction['variables'][number], value: any) => {
      ...
      const actionIndex = _.findIndex(clonedContent.actions, n => n.id === id)
         return _.update(clonedContent, `actions[${actionIndex}].variables[${_.findIndex(clonedContent.actions[actionIndex].variables, n => n.name === variableName)}].${target}`, v => value)
      })
   }, [dynamicParsedContent])
   ...
   return {
      ...
      mutations,
      mainFragment: dynamicParsedContent.mainFragment,
